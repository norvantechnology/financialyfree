...
| **News** (`news`) | `LIVE_FETCH` | Click "Force refresh now" at `/admin/data-integrity` or inspect `/techno-funda?tab=news`; verify NSE/BSE filings + ET headlines and timestamp. | | 2026-09-16 | Merges NSE `corporate-announcements?index=equities`, BSE `AnnSubCategoryGetData/w` (14-day single-day walk), and ET RSS. Cached in `data_source_health.rawResponseSnippet`. Seeds Order Tracker order-win detection. |
| **Order Tracker** (`order_tracker`) | `LIVE_FETCH` (derived) | Open `/techno-funda?tab=orders` or `GET /api/v1/techno-funda/orders?refresh=true`; confirm non-zero `contractValueCr` when PDF/headline discloses ₹ Cr (e.g. Suratwwala annexure ₹69 Cr, Patels headline ₹226 Cr). | | 2026-09-16 | **No dedicated DB table.** `getOrderTracker()` uses `news` headlines + BSE `AttachLive`/`AttachHis` PDF annexure extraction (`pdfjs-dist` / `pdftotext`). Full params & UI/DB mapping: [`docs/integrations/order-tracker-bse-nse.md`](integrations/order-tracker-bse-nse.md) and PLATFORM §5.4 / §6.6 / §6.11. |
...
- **Order Tracker live API**: `GET http://localhost:3001/api/v1/techno-funda/orders?refresh=true`
- **Order Tracker integration doc**: [`integrations/order-tracker-bse-nse.md`](integrations/order-tracker-bse-nse.md)
